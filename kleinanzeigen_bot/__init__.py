...
from .utils import apply_defaults, is_frozen, pause, pluralize, safe_get
from .selenium_support import SeleniumSupport
...
class KleinanzeigenBot(SeleniumSupport):
    ...
    def __init__(self):
        ...
        self.config_file_path:str = os.path.join(os.getcwd(), "config.yaml")

        self.categories:Dict[str, str] = {}

        self.file_log:logging.FileHandler = None
        self.log_file_path:str = os.path.join(os.getcwd(), self.__module__ + ".log")
    ...
    def show_help(self) -> None:
        ...
        print(textwrap.dedent(f"""\
            ...
            - publish - (re-)publishes ads (default command)
    ...
    def parse_args(self, args:Iterable[str]) -> None:
        ...
        if len(arguments) == 1:
            if not arguments[0] in ("help", "version", "publish", "verify"):
                LOG.error("Unknown command: %s", arguments[0])
                sys.exit(2)
    ...
    def load_ads(self, exclude_inactive = True, exclude_undue = True) -> Iterable[Dict[str, Any]]:
        ...
        for ad_file in sorted(ad_files):
            ...
            apply_defaults(ad_cfg, self.config["ad_defaults"], lambda k, _: k != "description")
            ...
            if exclude_undue:
                ...
                if last_updated_on:
                    ...
                    if ad_age.days <= ad_cfg["renewal_interval"]:
                        LOG.info(" -> skipping. last published %d days ago. renewal only every %s days required", ad_age.days, ad_cfg["renewal_interval"])
                        continue
            ...
            def assert_one_of(path:str, allowed:Iterable):
                assert safe_get(ad_cfg, *path.split(".")) in allowed, f'-> property [{path}] must be one of: {allowed} @ [{ad_file}]'

            def assert_min_len(path:str, minlen:int):
                assert len(safe_get(ad_cfg, *path.split("."))) >= minlen, f'-> property [{path}] must be at least {minlen} characters long @ [{ad_file}]'

            def assert_has_value(path:str):
                assert safe_get(ad_cfg, *path.split(".")), f'-> property [{path}] not specified @ [{ad_file}]'
            ...
            assert_has_value("renewal_interval")
            ...
            if ad_cfg["images"]:
                ...
                for image_pattern in ad_cfg["images"]:
                    for image_file in glob.glob(image_pattern, root_dir = os.path.dirname(ad_file), recursive = True):
                        ...
                        assert image_file_ext.lower() in (".gif", ".jpg", ".jpeg", ".png"), f'Unsupported image file type [{image_file}]'
                        if os.path.isabs(image_file):
                            images.add(image_file)
                        else:
                            images.add(os.path.join(os.path.dirname(ad_file), image_file))
                assert images or not ad_cfg["images"], f'No images found for given file patterns {ad_cfg["images"]} at {os.getcwd()}'
    ...
    def load_config(self) -> None:
        ...
        assert self.config["login"]["username"], f'[login.username] not specified @ [{self.config_file_path}]'
        assert self.config["login"]["password"], f'[login.password] not specified @ [{self.config_file_path}]'

        self.browser_arguments = self.config["chrome"]["arguments"]
        self.browser_binary_location = self.config["chrome"]["binary_location"]
    ...
    def handle_captcha_if_present(self, captcha_element_id:str, msg:str) -> None:
        ...
        LOG.warning("############################################")
        LOG.warning("# Captcha present! Please answer and close the captcha, %s", msg)
        LOG.warning("############################################")
    ...
    def publish_ad(self, ad_file, ad_cfg: Dict[str, Any], ad_cfg_orig: Dict[str, Any]) -> None:
        ...
        if ad_cfg["category"]:
            ...
            assert is_category_auto_selected, f'No category specified in [{ad_file}] and automatic category detection failed'
        ...
        for image in ad_cfg["images"]:
            ...
            assert previous_uploaded_images_count < count_uploaded_images(), f"couldn't upload image [{image}] within 60 seconds"
