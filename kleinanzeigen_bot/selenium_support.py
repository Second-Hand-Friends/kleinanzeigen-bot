...
from .utils import is_frozen, pause

LOG:Final[logging.Logger] = logging.getLogger("kleinanzeigen_bot.selenium_support")
...
class SeleniumSupport:
    ...
    def create_webdriver_session(self) -> None:
        ...
        if shutil.which(DEFAULT_CHROMEDRIVER_PATH):
            ...
            if self.browser_binary_location:
                chrome_type, chrome_version = self.get_browser_info(self.browser_binary_location)
            else:
                chrome_type, chrome_version = self.get_browser_info_from_os()
        ...
        if is_frozen():
            import certifi, pathlib  # pylint: disable=import-outside-toplevel

            read_text_orig = pathlib.Path.read_text

            def read_text_new(self, encoding = None, errors = None):
                if self.name == "cacert.pem":
                    return certifi.core.contents()
                path = str(self)
    ...
    def get_browser_info(self, executable_path: str):
    ...
    def get_browser_info_from_os(self):
    ...
    def web_request(self, url:str, method:str = "GET", valid_response_codes:Iterable[int] = [200], headers:Dict[str, str] = None) -> Dict[str, Any]:
        ...
        """)
        assert response["statusCode"] in valid_response_codes, \
