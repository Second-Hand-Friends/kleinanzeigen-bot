...
class SeleniumMixin:
    ...
    def create_webdriver_session(self) -> None:
        ...
        if shutil.which(DEFAULT_CHROMEDRIVER_PATH):
            ...
            LOG.info("Using Browser: %s %s [%s]", chrome_type.upper(), chrome_version, chrome_path)
    ...
    def get_browser_version(self, executable_path: str) -> tuple[ChromeType, str]:
        match ChromeDriverManagerUtils.os_name():
            ...
            case OSType.LINUX:
                version_cmd = ChromeDriverManagerUtils.linux_browser_apps_to_cmd(executable_path)

            case _:
                version_cmd = executable_path + " --version"

        filename = os.path.basename(executable_path).lower()
        if "chromium" in filename:
            return (ChromeType.CHROMIUM, ChromeDriverManagerUtils.read_version_from_cmd(version_cmd, ChromeDriverManagerUtils.PATTERN[ChromeType.CHROMIUM]))
        if "edge" in filename:
            return (ChromeType.MSEDGE, ChromeDriverManagerUtils.read_version_from_cmd(version_cmd, ChromeDriverManagerUtils.PATTERN[ChromeType.MSEDGE]))
        return (ChromeType.GOOGLE, ChromeDriverManagerUtils.read_version_from_cmd(version_cmd, ChromeDriverManagerUtils.PATTERN[ChromeType.GOOGLE]))
        ...
        match ChromeDriverManagerUtils.os_name():
            ...
            case OSType.MAC:
                browser_paths = [
                    ...
                    "/Applications/Google Chrome.app.app/Contents/MacOS/Google Chrome",
