...
class TimeoutConfig(ContextualModel):
    ...
    default:float = Field(default = 1.5, ge = 0.0, description = "Baseline timeout for DOM interactions.")
    page_load:float = Field(default = 10.0, ge = 1.0, description = "Page load timeout for web_open.")
    captcha_detection:float = Field(default = 1.0, ge = 0.1, description = "Timeout for captcha iframe detection.")
    sms_verification:float = Field(default = 2.0, ge = 0.1, description = "Timeout for SMS verification prompts.")
    gdpr_prompt:float = Field(default = 6.0, ge = 1.0, description = "Timeout for GDPR/consent dialogs.")
    publishing_result:float = Field(default = 300.0, ge = 10.0, description = "Timeout for publishing result checks.")
    publishing_confirmation:float = Field(default = 12.0, ge = 1.0, description = "Timeout for publish confirmation redirect.")
    pagination_initial:float = Field(default = 6.0, ge = 1.0, description = "Timeout for initial pagination lookup.")
    pagination_follow_up:float = Field(default = 3.0, ge = 1.0, description = "Timeout for subsequent pagination navigation.")
    quick_dom:float = Field(default = 1.0, ge = 0.1, description = "Generic short timeout for transient UI.")
    update_check:float = Field(default = 6.0, ge = 1.0, description = "Timeout for GitHub update checks.")
    chrome_remote_probe:float = Field(default = 1.0, ge = 0.1, description = "Timeout for local remote-debugging probes.")
    chrome_remote_debugging:float = Field(default = 3.0, ge = 1.0, description = "Timeout for remote debugging API calls.")
    chrome_binary_detection:float = Field(default = 6.0, ge = 1.0, description = "Timeout for chrome --version subprocesses.")
    retry_enabled:bool = Field(default = False, description = "Enable built-in retry/backoff for DOM operations.")
    retry_max_attempts:int = Field(default = 1, ge = 1, description = "Max retry attempts when retry is enabled.")
    retry_backoff_factor:float = Field(default = 1.2, ge = 1.0, description = "Exponential factor applied per retry attempt.")
