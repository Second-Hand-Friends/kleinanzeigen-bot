...
class KleinanzeigenBot(WebScrapingMixin):  # noqa: PLR0904
    ...
    async def _has_logged_out_cta(self, *, log_timeout:bool = True) -> bool:
        ...
        try:
            ...
            cta_text = await self.extract_visible_text(cta_element)
            if cta_text.strip():
                matched_selector_display = (
    ...
    async def publish_ad(
        ...
        await _publishing_flow.PublishingFormFlow(
            web = self, root_url = self.root_url, ad_defaults = self.config.ad_defaults
        ).fill_ad_form(ad_file, ad_cfg, mode)
